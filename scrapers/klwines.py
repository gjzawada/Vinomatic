"""
K&L Wine Merchants - their search results page is server-rendered.
"""
import logging
...
URLS = [
    BASE + "/Products?&productTypeID=1&ms=5&order=1",
    BASE + "/Products?&productTypeID=1&ms=5",
    BASE + "/Products?keywords=wine+sale&productTypeID=1",
]
...
def scrape_klwines() -> list:
    ...
    for url in URLS:
        ...
        logger.info(f"[K&L] classes: {sorted(classes)[:20]}")

        cards = (soup.select(".tf-product-container") or soup.select(".result") or
                 soup.select("[class*='product-result']") or soup.select("[class*='result-item']") or
                 soup.select("li[class*='result']") or soup.select(".search-result") or
                 soup.select("[class*='wine-result']") or soup.select("li[class*='product']"))
    ...
    logger.info(f"[K&L] {len(wines)} wines")
    return wines
